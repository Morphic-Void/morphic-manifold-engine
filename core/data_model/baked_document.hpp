...
public:
    ...
    CBakedDocument(const void* bytes, std::size_t size) noexcept;
    ...
    [[nodiscard]] EJsonNodeType node_type(CBakedNodeIndex node) const noexcept;
    [[nodiscard]] bool boolean_value(CBakedNodeIndex node, bool& value) const noexcept;
    [[nodiscard]] bool integer_value(CBakedNodeIndex node, std::int64_t& value) const noexcept;
    [[nodiscard]] bool floating_point_value(CBakedNodeIndex node, double& value) const noexcept;
    [[nodiscard]] CStringView string_value(CBakedNodeIndex node) const noexcept;
    [[nodiscard]] CStringView string_value(CStringValueId value) const noexcept;
    [[nodiscard]] CStringView property_name(CPropertyNameId name) const noexcept;
    [[nodiscard]] CBakedNodeIndex parent(CBakedNodeIndex node) const noexcept;
    [[nodiscard]] CPropertyNameId name_in_parent(CBakedNodeIndex node) const noexcept;
    [[nodiscard]] CBakedNodeIndex previous_sibling(CBakedNodeIndex node) const noexcept;
    [[nodiscard]] CBakedNodeIndex next_sibling(CBakedNodeIndex node) const noexcept;
    [[nodiscard]] std::uint32_t child_count(CBakedNodeIndex container) const noexcept;
    [[nodiscard]] CBakedNodeIndex first_child(CBakedNodeIndex container) const noexcept;
    [[nodiscard]] CBakedNodeIndex last_child(CBakedNodeIndex container) const noexcept;
    [[nodiscard]] CBakedNodeIndex array_at(CBakedNodeIndex array, std::uint32_t index) const noexcept;
    [[nodiscard]] CBakedNodeIndex object_child(CBakedNodeIndex object, CPropertyNameId name) const noexcept;
    [[nodiscard]] CBakedNodeIndex object_child(CBakedNodeIndex object, const CStringView& name) const noexcept;
    [[nodiscard]] bool check_integrity() const noexcept;

private:
    [[nodiscard]] static bool is_array(EJsonNodeType type) noexcept;
    [[nodiscard]] static bool is_container(EJsonNodeType type) noexcept;
    [[nodiscard]] static std::uint32_t crc(const std::uint8_t* bytes, std::size_t byte_count) noexcept;
    [[nodiscard]] static bool range(std::uint32_t offset, std::uint64_t byte_count, std::uint32_t total_size) noexcept;
    ...
    [[nodiscard]] CStringView string_from(std::uint32_t references_offset, std::uint32_t reference_count,
        std::uint32_t bytes_offset, std::uint32_t bytes_size, std::uint32_t string_id) const noexcept;
...
{
    const CBakedNode* const node_record = node_slot(node);
    if (!node_record || (node_record->type != EJsonNodeType::boolean)) return false;
...
{
    const CBakedNode* const node_record = node_slot(node);
    if (!node_record || (node_record->type != EJsonNodeType::integer)) return false;
...
{
    const CBakedNode* const node_record = node_slot(node);
    if (!node_record || (node_record->type != EJsonNodeType::floating_point)) return false;
...
{
    return string_from(m_header ? m_header->value_refs_offset : 0u, m_header ? m_header->value_ref_count : 0u,
        m_header ? m_header->value_bytes_offset : 0u, m_header ? m_header->value_bytes_size : 0u, value.query_value());
}

inline CStringView CBakedDocument::property_name(const CPropertyNameId name) const noexcept
{
    return string_from(m_header ? m_header->name_refs_offset : 0u, m_header ? m_header->name_ref_count : 0u,
        m_header ? m_header->name_bytes_offset : 0u, m_header ? m_header->name_bytes_size : 0u, name.query_value());
}
...
inline CBakedNodeIndex CBakedDocument::previous_sibling(const CBakedNodeIndex node) const noexcept
{
    ...
    if (!parent_record || (node.query_value() <= parent_record->first_child_index) ||
        (node.query_value() >= (parent_record->first_child_index + parent_record->child_count))) return CBakedNodeIndex{};
...
inline CBakedNodeIndex CBakedDocument::next_sibling(const CBakedNodeIndex node) const noexcept
{
    ...
    const CBakedNode* const parent_record = node_record ? node_slot(node_record->parent) : nullptr;
    if (!parent_record || (node.query_value() < parent_record->first_child_index) ||
        ((node.query_value() + 1u) >= (parent_record->first_child_index + parent_record->child_count))) return CBakedNodeIndex{};
    return CBakedNodeIndex{ node.query_value() + 1u };
}
...
{
    const CBakedNode* const node_record = node_slot(container);
    return (node_record && is_container(node_record->type) && node_record->child_count) ? CBakedNodeIndex{ node_record->first_child_index } : CBakedNodeIndex{};
}

inline CBakedNodeIndex CBakedDocument::last_child(const CBakedNodeIndex container) const noexcept
{
    const CBakedNode* const node_record = node_slot(container);
    return (node_record && is_container(node_record->type) && node_record->child_count) ? CBakedNodeIndex{ static_cast<std::uint32_t>(node_record->first_child_index + node_record->child_count - 1u) } : CBakedNodeIndex{};
}
...
inline CBakedNodeIndex CBakedDocument::array_at(const CBakedNodeIndex array, const std::uint32_t index) const noexcept
{
    ...
    return (node_record && is_array(node_record->type) && (index < node_record->child_count)) ? CBakedNodeIndex{ static_cast<std::uint32_t>(node_record->first_child_index + index) } : CBakedNodeIndex{};
}

inline bool CBakedDocument::is_array(const EJsonNodeType type) noexcept { return (type == EJsonNodeType::array) || (type == EJsonNodeType::recovered_duplicate_array); }
...
inline CBakedNodeIndex CBakedDocument::object_child(const CBakedNodeIndex object, const CPropertyNameId name) const noexcept
{
    ...
    if (!object_record || (object_record->type != EJsonNodeType::object) || !name.is_valid()) return CBakedNodeIndex{};
...
{
    const CBakedNode* const object_record = node_slot(object);
    if (!object_record || (object_record->type != EJsonNodeType::object) || (name.string() == nullptr)) return CBakedNodeIndex{};
    for (std::uint32_t child_offset = 0u; child_offset < object_record->child_count; ++child_offset)
    {
        const CBakedNodeIndex child{ object_record->first_child_index + child_offset };
        const CStringView child_name = property_name(name_in_parent(child));
        if ((child_name.length() == name.length()) && (std::memcmp(child_name.string(), name.string(), child_name.length()) == 0)) return child;
    }
    return CBakedNodeIndex{};
}
...
inline std::uint32_t CBakedDocument::crc(const std::uint8_t* bytes, std::size_t byte_count) noexcept
{
    ...
    {
        value ^= *bytes;
        for (std::uint32_t bit = 0u; bit < 8u; ++bit) value = (value >> 1u) ^ ((value & 1u) ? 0xEDB88320u : 0u);
    }
    return ~value;
}
...
{
    if (!m_header || !node.is_valid() || (node.query_value() >= m_header->node_count)) return nullptr;
    return reinterpret_cast<const CBakedNode*>(m_bytes + m_header->nodes_offset) + node.query_value();
}

inline CStringView CBakedDocument::string_from(const std::uint32_t references_offset, const std::uint32_t reference_count,
    const std::uint32_t bytes_offset, const std::uint32_t bytes_size, const std::uint32_t string_id) const noexcept
{
    if (!m_header || (string_id == 0u) || (string_id >= reference_count)) return CStringView{};
    const CBakedStringRef& reference = reinterpret_cast<const CBakedStringRef*>(m_bytes + references_offset)[string_id];
    return ((reference.offset <= bytes_size) && (reference.length <= (bytes_size - reference.offset))) ? CStringView{ reinterpret_cast<const char*>(m_bytes + bytes_offset + reference.offset), reference.length } : CStringView{};
}

inline bool CBakedDocument::validate(const std::uint8_t* const bytes, const std::size_t byte_count) noexcept
{
    if (!bytes || (byte_count < sizeof(CBakedDocumentHeader)) || (byte_count > std::numeric_limits<std::uint32_t>::max())) return false;
    ...
    if ((header.magic != k_magic) || (header.version != k_version) || (header.header_size != sizeof(header)) ||
        ...
        (header.root_index >= header.node_count)) return false;
    if ((reinterpret_cast<std::uintptr_t>(bytes) % alignof(CBakedNode)) != 0u ||
        ...
        (header.value_refs_offset % alignof(CBakedStringRef)) != 0u) return false;
    if (!range(header.nodes_offset, std::uint64_t(header.node_count) * sizeof(CBakedNode), header.total_size) ||
        ...
        (header.name_ref_count == 0u) || (header.value_ref_count == 0u)) return false;
    if (crc(bytes + header.header_size, byte_count - header.header_size) != header.payload_crc) return false;
    ...
    if ((property_name_references[0].offset != 0u) || (property_name_references[0].length != 0u) ||
        (string_value_references[0].offset != 0u) || (string_value_references[0].length != 0u)) return false;
    for (std::uint32_t reference_index = 1u; reference_index < header.name_ref_count; ++reference_index)
    {
        ...
        if ((reference.offset > header.name_bytes_size) || (reference.length > (header.name_bytes_size - reference.offset))) return false;
    }
    for (std::uint32_t reference_index = 1u; reference_index < header.value_ref_count; ++reference_index)
    {
        ...
        if ((reference.offset > header.value_bytes_size) || (reference.length > (header.value_bytes_size - reference.offset))) return false;
    }
    ...
    for (std::uint32_t node_index = 1u; node_index < header.node_count; ++node_index)
    {
        ...
        if ((node.type <= EJsonNodeType::invalid) || (node.type > EJsonNodeType::recovered_duplicate_array) ||
            (node.parent.query_value() >= header.node_count) || (node.name_in_parent.query_value() >= header.name_ref_count)) return false;
        if ((node.type == EJsonNodeType::string) && (node.payload.string_value.query_value() >= header.value_ref_count)) return false;
        if (!is_container(node.type))
        {
            if ((node.child_count != 0u) || (node.first_child_index != 0u)) return false;
            continue;
        }
        if (node.child_count && ((node.first_child_index == 0u) || (node.first_child_index >= header.node_count) ||
            (node.child_count > (header.node_count - node.first_child_index)))) return false;
        for (std::uint32_t child_offset = 0u; child_offset < node.child_count; ++child_offset)
        {
            if (nodes[node.first_child_index + child_offset].parent.query_value() != node_index) return false;
        }
    }
...
inline bool CBakedDocumentBlock::build_from(const CBakedDocumentBuilder& source) noexcept
{
    ...
    std::uint64_t at=sizeof(CBakedDocumentHeader);
    const std::uint32_t no=static_cast<std::uint32_t>((at+alignof(CBakedNode)-1u)&~(alignof(CBakedNode)-1u));
    at=std::uint64_t(no)+std::uint64_t(node_count)*sizeof(CBakedNode);
    const std::uint32_t nro=static_cast<std::uint32_t>((at+alignof(CBakedStringRef)-1u)&~(alignof(CBakedStringRef)-1u));
    at=std::uint64_t(nro)+std::uint64_t(nc)*sizeof(CBakedStringRef);
    const std::uint32_t nbo=static_cast<std::uint32_t>(at);
    at+=nb;
    const std::uint32_t vro=static_cast<std::uint32_t>((at+alignof(CBakedStringRef)-1u)&~(alignof(CBakedStringRef)-1u));
    at=std::uint64_t(vro)+std::uint64_t(vc)*sizeof(CBakedStringRef);
    const std::uint32_t vbo=static_cast<std::uint32_t>(at);
    at+=vb;
    ...
    CBakedDocumentHeader* const h=reinterpret_cast<CBakedDocumentHeader*>(staged.data()); *h={CBakedDocument::k_magic,CBakedDocument::k_version,static_cast<std::uint16_t>(sizeof(CBakedDocumentHeader)),source.m_contains_recovered_duplicate_arrays ? CBakedDocument::k_flag_recovered_duplicate_arrays : 0u,static_cast<std::uint32_t>(at),source.m_root.query_value(),no,node_count,nro,nc,nbo,static_cast<std::uint32_t>(nb),vro,vc,vbo,static_cast<std::uint32_t>(vb),0u};
    std::memcpy(staged.data()+no,source.m_nodes.data(),std::size_t(node_count)*sizeof(CBakedNode));
    CBakedStringRef* const property_name_references=reinterpret_cast<CBakedStringRef*>(staged.data()+nro);
    std::uint32_t property_name_cursor=0u;
    for(std::uint32_t string_index=1u;string_index<nc;++string_index) { const CStringView string=source.m_property_names.view(string_index); property_name_references[string_index]={property_name_cursor,static_cast<std::uint32_t>(string.length())}; std::memcpy(staged.data()+nbo+property_name_cursor,string.string(),string.length()); property_name_cursor+=static_cast<std::uint32_t>(string.length()); }
    CBakedStringRef* const string_value_references=reinterpret_cast<CBakedStringRef*>(staged.data()+vro);
    std::uint32_t string_value_cursor=0u;
    for(std::uint32_t string_index=1u;string_index<vc;++string_index) { const CStringView string=source.m_string_values.view(string_index); string_value_references[string_index]={string_value_cursor,static_cast<std::uint32_t>(string.length())}; std::memcpy(staged.data()+vbo+string_value_cursor,string.string(),string.length()); string_value_cursor+=static_cast<std::uint32_t>(string.length()); }
    h->payload_crc=CBakedDocument::crc(staged.data()+sizeof(*h),staged.size()-sizeof(*h));
