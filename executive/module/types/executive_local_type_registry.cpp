
//  Copyright (c) 2026 Ritchie Brannan / Morphic Void Limited
//  License: MIT (see LICENSE file in repository root)
//
//  File:    executive_local_type_registry.cpp
//  Authors: Ritchie Brannan / OpenAI Codex
//  Date:    12 Aug 26

#include "executive/module/types/executive_local_type_registry.hpp"

namespace executive
{

namespace local_types
{
constexpr local_type_registry::SLocalTypeRegistration s_types[] =
{
#define MV_LOCAL_TYPE(name, cpp_type) \
    { executive_local_type_ids::name, executive_local_type_ids::name##_index, local_type_registry::make_name(#name) },
#include "executive/module/types/executive_local_type_ids.def"
#undef MV_LOCAL_TYPE
};

constexpr local_type_registry::SLocalTypeRegistryView s_view{ s_types, static_cast<std::uint32_t>(executive_local_type_ids::k_count) };
constexpr erased_owner_operations::SCategoryView s_erased_owner_operations_view{};
}   //  namespace local_types

const local_type_registry::SLocalTypeRegistryView& MV_STD_ABI_CALL local_type_registry_view() noexcept
{
    return local_types::s_view;
}

const erased_owner_operations::SCategoryView& MV_STD_ABI_CALL local_erased_owner_operations_view() noexcept
{
    return local_types::s_erased_owner_operations_view;
}

}   //  namespace executive
